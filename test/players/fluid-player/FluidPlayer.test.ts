...
describe('Test cases for players/fluid-player/FluidPlayer.js file', () => {
    ...
    beforeEach(() => {
        document.body.innerHTML =
            '<div id="' + elementId + '"><video id="' + videoPlayerId + '"></video>></div>';
    });
    ...
    describe('generatePlayerConfig method', () => {
        test('it should create FluidPlayerConfig object', () => {
            fluidPlayerObj.generatePlayerConfig(bid, elementId, genericConfiguration);
            expect(fluidPlayerObj.fluidPlayerConfig).toBeDefined();
        });

        test('it should throw error, if valid elementId is not provided', () => {
            expect(() => {
                fluidPlayerObj.generatePlayerConfig(bid, 'wrongID', genericConfiguration);
    ...
    describe('setupPlayer method', () => {
        ...
        afterEach(() => {
            (fluidPlayerObj.registerFluidPlayerCallbacks as jest.Mock).mockReset();
        });

        test('it should throw error, if no element with the given ID exists', () => {
            expect(() => {
                fluidPlayerObj.setupPlayer('not-existing-id');
            }).toThrowError('Please provide a valid video player ID.');
        });

        test('it should make element visible', () => {
            fluidPlayerObj.setupPlayer(videoPlayerId);
            expect(document.getElementById(videoPlayerId)!.style.display).toBe('block');
        });

        test('it should call fluidPlayer', () => {
            fluidPlayerObj.setupPlayer(videoPlayerId);
            expect(fluidPlayer).toHaveBeenNthCalledWith(
                1,
                videoPlayerId,
                fluidPlayerObj.fluidPlayerConfig
            );
        });
    ...
    describe('registerFluidPlayerCallbacks method', () => {
        test('it should not throw', () => {
            ...
            fluidPlayer.registerFluidPlayerCallbacks();
        });
    });
